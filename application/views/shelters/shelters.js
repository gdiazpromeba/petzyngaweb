...
//			 $(".pagination").jqPagination('option', 'current_page',  $scope.storage.page);
	  ...
	  $scope.readPrimerasAreas = function(country){
		  var url = Global.dirCms + "/svc/conector/areas.php/selPrimerasAreasSheltersJson?country=" + country + '&addFirstAll=true ';
		  $log.info('leyendo primera áreas.\nLa url es=' + url)
		  $http.get(url).
	  ...
	  $scope.readSegundasAreas = function(country, firstArea){
		  var url = Global.dirCms + "/svc/conector/areas.php/selSegundasAreasSheltersJson?country=" + country + '&firstArea=' + firstArea  + '&addFirstAll=true ';
		  $http.get(url).
		    success(function(data, status, headers, config) {
	  ...
	  $scope.callService = function(reset){
	    ...
	    $scope.buildUrl=function(reset){
	    	var url=  Global.dirCms + "/svc/conector/shelters" +  $scope.storage.country2.charAt(0).toUpperCase() + $scope.storage.country2.slice(1)  + ".php/seleccionaUniversal";
	    ...
	    $scope.buttonClick=function(){
		  ...
		  $scope.$watch(function(scope) { return scope.storage.formParams.dogBreedName },
	              function(newValue, oldValue) {
			          $log.info("newValue=" + newValue)
	              }
	     );		  
		  
//		  $scope.$watch('storage.formParams.specialBreedId', function() {
//		       $scope.$apply();
//		  });		  
  ...
  app.controller('RelatedDogBreedsCtrl', ['$scope',  '$rootScope', '$http',  function($scope, $rootScope, $http){
	  ...
	  $scope.setShelterId = function(id){
		  $scope.shelterId = id;
		  var url= Global.dirCms + '/svc/conector/dogBreeds.php/selNombresPorShelter?shelterId=' + id;
		  $http.get(url).
		    success(function(data, status, headers, config) {
